Vue.component('md-input-number', {
 ...
 template: "<div class='input-field'>"
         ...
         + "<input v-if=hasPlaceholder :placeholder=computedPlaceholder :id=computedId type='number' :step=computedStep class='validate' :value='value' @input=\"$emit('input', $event.target.value)\">"
         + "<input v-else-if=!hasPlaceholder :id=computedId type='number' :step=computedStep class='validate' :value='value' @input=\"$emit('input', $event.target.value)\">"
